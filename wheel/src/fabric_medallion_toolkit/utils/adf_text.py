...
def extract_adf_text(adf_value) -> Optional[str]:
    """
    adf_value: either the ADF dict itself, or a JSON string containing one
    (as it typically arrives from a Bronze/Silver column that's already
    been through to_json). Returns plain text with paragraph breaks
    preserved as newlines, or None if there's nothing extractable.
    """
