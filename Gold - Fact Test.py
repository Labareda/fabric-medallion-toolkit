...
df = spark.sql(f"""
    ...
    SELECT
        ...
        -- Natural keys kept for lookup_missing_from safety net
        m.Test_Set_Id                               AS Test_Set_Id,
        COALESCE(lr.Latest_Status, 'NOT RUN')       AS Test_Status_Name,
        lr.executed_by_id                           AS Resource_Account_Id,
        -- Surrogate keys resolved by explicit dim joins (same pattern as Fact_Issue)
        dts.Test_Set_Key,
        ts.Test_Status_Key,
    ...
    LEFT JOIN {GOLD_SCHEMA}.dim_test_status ts
           ON ts.Test_Status_Name = COALESCE(lr.Latest_Status, 'NOT RUN')
